...
watch(
  ...
  (newTitle) => {
    document.title = newTitle || "thinhm";
  },
  { immediate: true },
);
...
const customJsRuntime = (() => {
  ...
  const ctx: CustomCtx = {
    ...
    emit(type: string, detail?: unknown) {
      window.dispatchEvent(new CustomEvent(`thinhm:${type}`, { detail }));
    },
  };
  ...
  const setRegister = () => {
    ...
    if (typeof w.thinhmCustomRegister === "function") return;
    w.thinhmCustomRegister = (h: unknown) => {
  ...
  const apply = async (code: string, agreed: boolean) => {
    ...
    (window as unknown as Record<string, unknown>).thinhmCustomCtx = ctx;
    ...
    const suffix = "\n;globalThis.thinhmCustomRegister?.(globalThis.thinhmCustom);";
    if (looksModule) {
    ...
    const adoptFromWindow = () => {
      const w = window as unknown as Record<string, unknown>;
      const fallback = (w.thinhmCustom as CustomHooks | undefined) || null;
...
<template>
  ...
  <audio id="thinhm-global-audio" style="display: none" crossorigin="anonymous"></audio>
</template>
