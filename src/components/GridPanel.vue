...
const fetchIp = async (force = false) => {
  const CACHE_KEY = `thinhm_ip_cache:${networkScope}`;
    ...
    >
      ...
      >
        <div class="flex flex-col lg:flex-row lg:justify-between items-center mb-10 gap-6 relative">
          ...
          >
            <form
              class="mx-auto shadow-lg hover:shadow-xl transition-shadow rounded-full bg-white/90 backdrop-blur-md border border-white/40 flex items-center p-1 thinhm-search-form"
              :style="{
                width: '100%',
                height: '41px',
                backgroundColor: `rgba(255, 255, 255, ${searchBgAlpha})`,
                '--thinhm-search-text-color': searchTextColor,
                '--thinhm-search-placeholder-color': searchPlaceholderColor,
              }"
            ...
            >
              <input
                ...
                autocomplete="off"
                autofocus
                class="h-full pl-6 pr-4 rounded-full bg-transparent border-0 outline-none thinhm-search-input"
              ...
              <div class="flex items-center justify-end" :style="{ width: '33.75%' }">
                <select
                  ...
                  aria-label="搜索引擎"
                  class="h-[34px] px-3 py-0 bg-transparent rounded-full border border-gray-200 focus:border-blue-400 outline-none thinhm-search-select"
          ...
          <div
            class="flex gap-3 flex-shrink-0 z-10 items-center transition-all duration-500 thinhm-handshake-signal"
...
.thinhm-search-input,
.thinhm-search-select {
  color: var(--thinhm-search-text-color, #111827);
}
.thinhm-search-input::placeholder {
  color: var(--thinhm-search-placeholder-color, rgba(107, 114, 128, 1));
}
