...
const getInitialCity = () => {
  ...
  try {
    const cachedCity = localStorage.getItem("thinhm_auto_city");
    if (cachedCity) {
...
const fetchWeather = async () => {
  try {
    ...
    if (props.widget?.data?.city) {
      ...
      if (cachedCity) {
        try {
          ...
          localStorage.removeItem("thinhm_auto_city");
        }
      }
    ...
    // 检查天气缓存 (缓存 20 分钟)
    const cachedWeather = localStorage.getItem(`thinhm_weather_${city}`);
    if (cachedWeather) {
      try {
        ...
        localStorage.removeItem(`thinhm_weather_${city}`);
      }
    }
