  ...
  if (audioRef.value) {
    ...
    try {
      ...
      if (msg.includes("TRACK_NOT_FOUND")) {
        ...
        await handleMissingTrack(track);
        return;
      }
      ...
      // Fallback: If blob failed, maybe try transcode to mp3?
      // Only if the error suggests format issue, but for now just show error.
    }
  }
};
...
// --- Storage Logic ---
const STORAGE_KEY = "thinhm_music_player_state";
...
onMounted(() => {
  const globalEl = document.getElementById("thinhm-global-audio") as HTMLAudioElement | null;
  if (globalEl) {
