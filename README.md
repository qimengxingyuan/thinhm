...
[![GitHub](https://img.shields.io/badge/GitHub-thinhm-181717?style=flat&logo=github&logoColor=white)](https://github.com/Garry-QD/thinhm)
[![Gitee](https://img.shields.io/badge/Gitee-thinhm-C71D23?style=flat&logo=gitee&logoColor=white)](https://gitee.com/gjx0808/thinhm)
[![Docker Image](https://img.shields.io/badge/Docker-qdnas%2Fthinhm-2496ED?style=flat&logo=docker&logoColor=white)](https://hub.docker.com/r/qdnas/thinhm)

thinhm 是一个轻量级、高度可定制的个人导航页与仪表盘系统。它基于 Vue 3 和 Express 构建，旨在为 NAS 用户、极客和开发者提供一个优雅的浏览器起始页。
交流QQ群:613835409
...
thinhm 后端集成了智能网络环境识别功能，能够根据用户的访问来源自动切换内外网访问策略，完美解决混合网络环境下的访问难题。
...
3. **docker-compose**
   ...
   services:
     thinhm:
       image: qdnas/thinhm:latest
       container_name: thinhm
...
thinhm 提供了强大的自定义能力，允许开发者或高级用户通过代码深度定制界面与交互。以下文档旨在帮助您（或您的 AI 助手）理解如何编写自定义内容。
...
**增强语法：**
thinhm 支持以下自定义标签，自动转换为媒体查询，方便响应式适配：
...
如果您正在开发多个复杂的子项目（如独立的 React/Vue 组件或工具应用），并希望将其集成到 thinhm 中，以下是推荐的最佳实践。

### 1. 运行时集成 (Runtime Integration)

#### 方案 A: Iframe 集成（最简单、隔离性最好）

这是将独立 Web 应用集成到 thinhm 的最推荐方式。

1. 将您的子项目独立构建并部署（或作为静态文件放入 thinhm）。
2. 在 thinhm 中添加 **"Iframe 网页"** 组件。
3. 填入子项目的 URL。
...
如果您希望子应用能与 thinhm 深度交互（如读取 Store 数据），可以将子应用构建为库 (Library)。

1. **放置资源**: thinhm 内置了一个公共静态目录 `server/public`。
...
2. **加载资源**: 在 **设置 -> 自定义 JS** 中编写加载脚本：
   ...
   import("/public/my-widget.js").then((module) => {
     // 初始化您的组件，传入 ctx
     module.default.init(thinhmCustomCtx);
   });
   ```
...
- 这样您可以同时启动 thinhm 主程序和多个子组件的开发服务器。
