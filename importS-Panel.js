...
async function importSunPanelConfig() {
  try {
    ...
    // 获取当前的 thinhm 配置，以便合并
    const currentthinhmConfigRes = await fetch(FLATNAS_API_URL);
    const currentthinhmConfig = await currentthinhmConfigRes.json();

    // 合并新的分组到现有配置中
    const updatedthinhmConfig = {
      ...currentthinhmConfig,
      groups: [...(currentthinhmConfig.groups || []), ...flatNasGroups],
    };
    ...
    const response = await fetch(FLATNAS_API_URL, {
      ...
      body: JSON.stringify(updatedthinhmConfig),
    });

    if (response.ok) {
      console.log("SunPanel 配置已成功导入到 thinhm。");
    } else {
