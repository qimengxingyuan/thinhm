...
setInterval(async () => {
  ...
  try {
    ...
    for (const c of containers) {
      ...
      // Skip thinhm itself to prevent suicide during update
      if (c.Image.includes("thinhm") || c.Names.some((n) => n.toLowerCase().includes("thinhm"))) {
        continue;
      }
...
try {
  ...
  app.get("/", (req, res) => {
    res.send("thinhm API Server Running");
  });
}
